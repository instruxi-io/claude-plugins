...
export async function browserSignIn({ base, resource, resources, scope, fetchImpl = fetch, onUrl, timeoutMs = 5 * 60_000 }) {
  ...
  try {
    const reg = await fetchImpl(meta.registration_endpoint, {
      ...
      body: JSON.stringify({ client_name: 'Enforcer (Claude Code)', redirect_uris: [redirectUri],
...
async function main(argv) {
  ...
  out('Usage: /enforcer:login [api-key <key> | status | logout]  — no argument opens a browser');
  process.exitCode = 2;
}
